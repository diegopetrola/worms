...
def rand_location(term:Terminal)->Location:
    """Return a random Location within the terminal"""
    return Location(x=randrange(1, term.width - 1),
                    y=randrange(1, term.height - 1))
...
def main():
    ...
    color_obs = term.color_rgb(0, 0, 0)
    ...
    # Obstacles
    num_obstacles = 4
    obstacles = [ rand_location(term) for _ in range(num_obstacles)]
    moving_obstacle = Location(5,5)
    # mov_obs_direction = right_of
    ...
    with term.hidden_cursor(), term.cbreak(), term.location():
        while inp not in (u'q', u'Q'):
            ...
            # Process obstacles here
            for obs in obstacles:
                echo(term.move_yx(*obs))
                echo(color_obs(u' '))

            if hit_any(head, obstacles):
                break
            
            echo(term.move_yx(*moving_obstacle))
            echo(color_bg(u' '))
            moving_obstacle = Location(moving_obstacle.y, moving_obstacle.x+1)
            echo(term.move_yx(*moving_obstacle))
            echo(color_obs(u' '))

            if hit(head, moving_obstacle):
                break
